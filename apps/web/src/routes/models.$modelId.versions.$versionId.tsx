...
export const Route = createFileRoute("/models/$modelId/versions/$versionId")({
...
function VersionDetailPage() {
  ...
  const [attributes, setAttributes] = useState<ModelVersion["attributes"]>(
    () => {
      const initialVersion = model.versions.find(
        (item) => item.id === versionId,
      );
      return initialVersion?.attributes || [];
    },
  );
  const [changelog, setChangelog] = useState(() => {
    const initialVersion = model.versions.find((item) => item.id === versionId);
    return initialVersion?.changelog || "";
  });
