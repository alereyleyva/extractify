...
function getExtractionFileKey(
  extractionId: string,
  sourceOrder: number,
  fileName: string,
): string {
  const sanitizedName = fileName.replace(/[^a-zA-Z0-9._-]/g, "_");
  return `extractions/${extractionId}/${sourceOrder}-${sanitizedName}`;
}
...
export async function uploadExtractionFile(
  ...
  const key = getExtractionFileKey(extractionId, sourceOrder, file.fileName);
  ...
  return `s3://${env.AWS_EXTRACTION_BUCKET}/${key}`;
}
